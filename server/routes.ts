...
import { insertYoutubePostSchema } from "@shared/schema";
...
- 출력은 JSON 형식으로 하며, title과 content를 분리하여 제공합니다.
...
위 지침에 따라 농구 게스트 모집 공지를 JSON 형식으로 작성해주세요. 반드시 유효한 JSON만 응답하세요.`;
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "당신은 농구 게스트 모집 공지를 자동으로 생성해주는 AI 챗봇입니다. 한국 표준시(KST)를 기준으로 날짜를 계산하고, 고정된 템플릿에 맞춰 JSON 형식으로 출력합니다. 반드시 유효한 JSON만 응답하세요.",
          },
          {
            role: "user",
            content: prompt,
